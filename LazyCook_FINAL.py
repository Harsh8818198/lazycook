...
class LLMConfig:
    ...
    backend: Literal["gemini", "gemma"] = "gemini"
    gemma_model_path: str = "google/gemma-2-2b-it"
    gemma_device: str = "cuda" if torch.cuda.is_available() else "cpu"
    gemma_max_length: int = 512  # ✅ INCREASED from 200
    gemini_model: str = "models/gemini-2.0-flash-exp"


class GemmaManager:
    """Singleton manager for Gemma model (shared across agents)"""
    _instance = None
    _initialized = False
    ...
    def __init__(self, model_name="google/gemma-2-2b-it", device=None):
        if GemmaManager._initialized or GemmaManager._initialization_failed:
            return
    ...
    def initialize(self) -> bool:
        """Initialize Gemma with RAM check"""
        if GemmaManager._initialized:
            logger.info("✅ Gemma already initialized")
            return True

        if GemmaManager._initialization_failed:
            logger.info("⚠️ Gemma initialization previously failed, skipping")
            return False
        ...
        try:
            ...
            if available_ram_gb < required_ram_gb:
                logger.warning(f"⚠️ Insufficient RAM: {available_ram_gb:.1f}GB < {required_ram_gb}GB")
                GemmaManager._initialization_failed = True
                return False
            ...
            GemmaManager._initialized = True
    ...
    def generate(self, prompt: str, temperature: float = 0.7, max_length: int = 200) -> str:
        """Generate response using Gemma with improved settings"""
        if not GemmaManager._initialized or self.model is None:
            raise RuntimeError("Gemma model not initialized!")

        # ✅ FIX: Improved tokenization and generation settings
        inputs = self.tokenizer(
            prompt,
            return_tensors="pt",
            truncation=True,
            max_length=2048  # Prevent input overflow
        ).to(self.device)
        ...
        with torch.no_grad():
            outputs = self.model.generate(
                **inputs,
                ...
                temperature=max(0.1, temperature),  # Prevent zero temp
                do_sample=True,  # Always sample for better responses
                top_p=0.9,
                top_k=50,
                repetition_penalty=1.2,  # ✅ NEW: Reduce repetition
                pad_token_id=self.tokenizer.pad_token_id or self.tokenizer.eos_token_id,
                eos_token_id=self.tokenizer.eos_token_id,
                no_repeat_ngram_size=3  # ✅ NEW: Prevent 3-gram repetition
            )

        # ✅ FIX: Better response extraction
        ...
        if response and not response[-1] in '.!?':
            ...
            if last_period > len(response) // 2:  # Only if sentence is substantial
...
class VectorStoreManager:
    """Manages vector embeddings for RAG using FAISS"""

    def __init__(self, model_name: str = "all-MiniLM-L6-v2", data_dir: str = "multi_agent_data"):
        self.model_name = model_name
        self.data_dir = Path(data_dir)
        self.embeddings_file = self.data_dir / "embeddings.faiss"
        self.metadata_file = self.data_dir / "embeddings_metadata.pkl"
        ...
        self.index = None
        self.metadata = []  # Store conversation IDs and text
    ...
    def _load_or_create_index(self):
        ...
        if self.embeddings_file.exists() and self.metadata_file.exists():
            try:
                self.index = faiss.read_index(str(self.embeddings_file))
    ...
    def add_conversation(self, conv_id: str, user_message: str, ai_response: str):
        ...
        logger.info(f"Added conversation {conv_id} to vector store (total: {self.index.ntotal})")
    ...
    def add_document(self, doc_id: str, filename: str, content: str):
        ...
        logger.info(f"Added document {filename} ({len(chunks)} chunks) to vector store")
    ...
    def search(self, query: str, top_k: int = 5) -> List[Tuple[dict, float]]:
        """Search for similar conversations/documents"""
    ...
    def save(self):
        """Save index to disk"""
        try:
            faiss.write_index(self.index, str(self.embeddings_file))
    ...
    def get_stats(self) -> dict:
        ...
        doc_count = sum(1 for m in self.metadata if m.get('type') == 'document')
        conv_count = len(self.metadata) - doc_count
...
class TextFileManager:
    def __init__(self, data_dir: str = "multi_agent_data", conversation_limit: int = 70):
        ...
        try:
            self.vector_store = VectorStoreManager(data_dir=str(self.data_dir))
    ...
    def get_conversation_context(self, user_id: str, limit: int = None, query: str = "") -> str:
        """
        SMART CONTEXT: Only retrieves conversations relevant to current query

        Args:
            user_id: User identifier
            limit: Ignored when using RAG (kept for backwards compatibility)
            query: Current user query to find similar past conversations

        Returns:
            String with ONLY relevant conversations (not all 70!)
        """

        # ✅ If we have RAG and a query, use smart retrieval
        if query and self.vector_store and self.vector_store.index.ntotal > 0:
            logger.info(f"🧠 Using RAG to find relevant conversations for: {query[:50]}...")
            return self._get_relevant_conversations_only(user_id, query)

        # ⚠️ Fallback: Return only last 3 conversations (not 70!)
        else:
            logger.warning("⚠️ No query provided or RAG unavailable, using last 3 convos")
            return self._get_traditional_context(user_id, limit=3)  # Limit to 3!

    def _get_relevant_conversations_only(self, user_id: str, query: str,
                                         top_k: int = 5,
                                         similarity_threshold: float = 0.3) -> str:
        """
        Core RAG function: Find ONLY similar conversations

        Args:
            query: Current user question
            top_k: Max number of relevant conversations to retrieve
            similarity_threshold: Minimum similarity score (0-1)

        Returns:
            Context with ONLY relevant past conversations
        """

        try:
            # 1. Search vector store for similar conversations
            results = self.vector_store.search(query, top_k=top_k * 2)  # Get extra for filtering

            if not results:
                logger.info("No similar conversations found, returning minimal context")
                return self._get_traditional_context(user_id, limit=1)

            # 2. Filter by user_id and similarity threshold
            relevant_results = []
            for metadata, distance in results:
                ...
                if similarity < similarity_threshold:
                    logger.debug(f"Skipping result with similarity {similarity:.2f}")
                    continue

                # Only include user's own conversations (optional - remove if you want all users)
                conv_id = metadata.get('conv_id', '')
                if user_id not in conv_id:
                    continue

                relevant_results.append((metadata, similarity))

                # Stop at top_k relevant results
                if len(relevant_results) >= top_k:
                    break

            if not relevant_results:
                logger.info("No conversations passed similarity threshold")
                return self._get_traditional_context(user_id, limit=1)

            # 3. Build compact context with ONLY relevant conversations
            context_parts = ["=== 🎯 RELEVANT PAST CONVERSATIONS ===\n"]

            for i, (metadata, similarity) in enumerate(relevant_results, 1):
                context_parts.append(f"\n--- Relevant Conversation {i} (Similarity: {similarity:.0%}) ---")
                context_parts.append(f"👤 USER: {metadata.get('user_message', '')}")
                context_parts.append(
                    f"🤖 ASSISTANT: {metadata.get('ai_response', '')[:300]}...")  # Truncate long responses

            # 4. Add current session context (last 1-2 convos for continuity)
            recent_convs = self.get_recent_conversations(user_id, 2)
            if recent_convs:
                context_parts.append("\n\n=== 📌 CURRENT SESSION (Last 2) ===")
                for i, conv in enumerate(recent_convs, 1):
                    context_parts.append(f"\n--- Recent {i} ---")
                    context_parts.append(f"👤 USER: {conv.user_message}")
                    context_parts.append(f"🤖 ASSISTANT: {conv.ai_response[:200]}...")

            # 5. Add document context if available
            docs_context = self.get_documents_context(user_id, limit=3, full_content=False)
            if docs_context:
                context_parts.append("\n\n=== 📄 RELEVANT DOCUMENTS ===")
                context_parts.append(docs_context[:500])  # Limit document context too
            ...
            # 6. Log token savings
            logger.info(f"✅ RAG Context: {len(final_context)} chars (~{len(final_context) // 4} tokens)")
            logger.info(f"💰 Token Savings: ~{130000 - len(final_context) // 4} tokens vs loading all 70 convos")

            return final_context

        except Exception as e:
            logger.error(f"RAG context retrieval failed: {e}")
            return self._get_traditional_context(user_id, limit=3)

    def _get_traditional_context(self, user_id: str, limit: int = 3) -> str:
        """
        Fallback: Traditional context loading (LIMITED to prevent token explosion)

        Args:
            limit: Max conversations (default 3, NOT 70!)
        """
        ...
        conversations = all_conversations[:limit]  # LIMIT TO 3!
        ...
        for i, conv in enumerate(conversations, 1):
            context_parts.append(f"\n--- Conversation {i} ---")
            context_parts.append(f"👤 USER: {conv.user_message}")
            context_parts.append(f"🤖 ASSISTANT: {conv.ai_response[:200]}...")  # Truncate
    ...
    def get_rag_context(self, query: str, top_k: int = 5) -> str:
        """
        Retrieve relevant context using RAG (vector similarity search)
        Returns formatted context string with most relevant conversations/documents
        """
        ...
        try:
            ...
            context_parts = ["=== 🔍 RELEVANT CONTEXT (RAG) ===\n"]
            ...
            for i, (metadata, distance) in enumerate(results, 1):
                ...
                if metadata.get('type') == 'document':
                    # Document chunk
                    context_parts.append(f"\n--- 📄 Document Match {i} (Similarity: {similarity:.2f}) ---")
                    context_parts.append(f"File: {metadata.get('filename', 'Unknown')}")
                    context_parts.append(f"Content: {metadata.get('text', '')[:300]}...")
                else:
                    # Conversation
                    context_parts.append(f"\n--- 💬 Conversation Match {i} (Similarity: {similarity:.2f}) ---")
                    context_parts.append(f"USER: {metadata.get('user_message', '')}")
                    context_parts.append(f"ASSISTANT: {metadata.get('ai_response', '')[:200]}...")

            context_parts.append("\n=== END OF RAG CONTEXT ===\n")

            logger.info(
                f"✅ RAG retrieved {len(results)} relevant items (avg similarity: {sum(1 - d / 10 for _, d in results) / len(results):.2f})")
            return "\n".join(context_parts)
        ...
        try:
            ...
            if file_type.startswith('text/'):
                ...
                try:
                    ...
                    content = "\n\n".join(content_parts) if content_parts else "[PDF - No text content]"
                    logger.info(f"PDF extracted: {len(reader.pages)} pages, {len(content)} characters")
                except Exception as e:
                    logger.error(f"Error extracting PDF: {e}")
                    content = f"[PDF - Error: {str(e)}]"
            elif file_type in ['text/markdown', 'text/csv']:
                with open(file_path, 'r', encoding='utf-8') as f:
                    content = f.read()
            else:
                content = f"[Binary file: {file_path.name} - not extractable]"
            ...
            document = Document(
                ...
                metadata={'original_path': str(file_path), 'processed_at': datetime.now().isoformat()}
            )

            self.save_document(document)

            # ADD TO VECTOR STORE (RAG)
            if self.vector_store and content and not content.startswith('['):
                try:
                    ...
                    self.vector_store.save()
                    logger.info(f"✅ Document added to vector store: {document.filename}")
                except Exception as e:
    ...
    def test_rag_context_reduction(self):
        """Test method to verify RAG is reducing token count"""

        user_id = "test_user"
        test_query = "How do I use Python decorators?"

        # Test OLD way (all 70 convos)
        old_context = self._get_traditional_context(user_id, limit=70)
        old_tokens = len(old_context) // 4

        # Test NEW way (RAG filtering)
        new_context = self._get_relevant_conversations_only(user_id, test_query, top_k=5)
        new_tokens = len(new_context) // 4

        print(f"\n{'=' * 60}")
        print(f"RAG CONTEXT REDUCTION TEST")
        print(f"{'=' * 60}")
        print(f"OLD Method (all 70 convos): {old_tokens:,} tokens")
        print(f"NEW Method (RAG top-5):     {new_tokens:,} tokens")
        print(
            f"Token Reduction:            {old_tokens - new_tokens:,} tokens ({(1 - new_tokens / old_tokens) * 100:.1f}% saved)")
        print(f"{'=' * 60}\n")
...
# --- AI Agent ---
class AIAgent:
    """Fixed AI Agent with proper backend selection"""
    _gemma_manager = None
    _gemini_configured = False

    def __init__(self, api_key: str, role, temperature: float, llm_config: LLMConfig):
        ...
        if self.llm_config.backend == "gemini":
            self._init_gemini()
        else:  # gemma
            self._init_gemma()
    ...
    def _init_gemini(self):
        ...
        try:
            if not AIAgent._gemini_configured:
                genai.configure(api_key=self.api_key)
                AIAgent._gemini_configured = True

            self.model = genai.GenerativeModel(
                model_name=self.llm_config.gemini_model,
                generation_config={
            ...
            logger.info(f"✅ {self.role.value} agent using Gemini")
    ...
    @log_errors
    async def process(self, user_query: str, context: str = "",
                      previous_iteration: Dict = None,
                      file_manager=None) -> AgentResponse:  # ADD file_manager parameter
        if self.role == AgentRole.GENERATOR:
            return await self._generate_solution(user_query, context, file_manager)
    ...
    async def _generate_with_backend(self, prompt: str) -> str:
        ...
        if self.llm_config.backend == "gemini":
            ...
            if self.gemma_manager is None:
                raise RuntimeError("Gemma manager not initialized!")

            # ✅ FIX: Format prompt for Gemma's instruction format
            ...
            try:
                response = await asyncio.wait_for(
                    loop.run_in_executor(
                        None,
                        self.gemma_manager.generate,
                        formatted_prompt,  # Use formatted prompt
                        self.temperature,
                        self.llm_config.gemma_max_length
                    ),
                    timeout=90.0  # Increased timeout
                )
                return response
            except asyncio.TimeoutError:
                logger.error("❌ Gemma generation timeout after 90s")
                return "Error: Gemma model timed out"

    def _format_gemma_prompt(self, prompt: str) -> str:
        """Format prompt for Gemma's instruction-following format"""

        # Gemma uses this format:
        # <start_of_turn>user
        # {instruction}
        # <end_of_turn>
        # <start_of_turn>model
    ...
    def _init_gemma(self):
        """Initialize Gemma backend with fallback to Gemini"""
        # Use shared Gemma manager
        if AIAgent._gemma_manager is None:
            AIAgent._gemma_manager = GemmaManager()
            success = AIAgent._gemma_manager.initialize()

            if not success:
                logger.warning(f"⚠️ {self.role.value} agent falling back to Gemini")
                self.llm_config.backend = "gemini"
                self._init_gemini()
                return

        # Check if Gemma is actually initialized
        if not GemmaManager._initialized:
            logger.warning(f"⚠️ Gemma not available, {self.role.value} using Gemini")
            self.llm_config.backend = "gemini"
            self._init_gemini()
            return

        self.gemma_manager = AIAgent._gemma_manager
        logger.info(f"✅ {self.role.value} agent using Gemma")


    @log_errors
    async def _generate_solution(self, user_query: str, context: str, file_manager=None) -> AgentResponse:
        """Generate solution with RAG-enhanced context"""

        # Get RAG context if file_manager provided
        rag_context = ""
        if file_manager and hasattr(file_manager, 'get_rag_context'):
            rag_context = file_manager.get_rag_context(user_query, top_k=3)

        prompt = f"""
        Role: Solution Generator Agent
        Task: Provide a comprehensive initial solution using conversation history AND relevant retrieved memories.

        🔍 RETRIEVED RELEVANT CONTEXT (RAG):
        {rag_context if rag_context else "[No similar past conversations/documents found]"}
        ...
        Instructions:
        1. PRIORITIZE information from RAG context if highly relevant
        2. Cross-reference with recent conversation history
        3. If RAG found similar past queries, build upon those solutions
        4. Provide a detailed, well-structured response
        5. Include practical examples where relevant
        6. Rate your confidence (0-1) based on context quality
        """

        try:
            response_text = await self._generate_with_backend(prompt)

            # Adjust confidence based on RAG context availability
            base_confidence = 0.8
            if rag_context and len(rag_context) > 100:
                base_confidence = 0.85  # Boost confidence with RAG
            ...
            return AgentResponse(
                ...
                metadata={
                    ...
                    "max_tokens": self.llm_config.gemma_max_length  # ✅ ADD THIS
                }
            )
        except Exception as e:
    ...
    async def _analyze_solution(self, user_query: str, context: str, previous_iteration: Dict) -> AgentResponse:
        ...
        # Simple heuristic analysis (no LLM call)
    ...
    async def _optimize_solution(self, user_query: str, context: str, previous_iteration: Dict) -> 'AgentResponse':
        """Optimize solution and return CLEAN TEXT (not JSON)"""
    ...
    Task: Create an improved solution based on analysis feedback.
    ...
    Instructions:
    1. Review context to maintain continuity
    2. Fix all identified errors
    3. Address gaps and improvements
    ...
    """
        try:
            ...
            # ✅ FIX: Parse JSON and extract clean solution
            parsed_data = self._parse_optimizer_response(response_text)

            if parsed_data:
                clean_solution = parsed_data.get("optimized_solution", response_text)
                confidence = parsed_data.get("confidence", 0.8)
                changes_made = parsed_data.get("changes_made", [])
                errors_fixed = parsed_data.get("errors_fixed", [])
                enhancements = parsed_data.get("enhancements", [])
            else:
                # Fallback: Use original response if JSON parsing fails
                clean_solution = response_text
                confidence = 0.7
                changes_made = []
                errors_fixed = []
                enhancements = []
            ...
            return AgentResponse(
                agent_role=self.role,
                content=clean_solution,  # ✅ CLEAN TEXT, not JSON!
                confidence=confidence,
                ...
                metadata={
                    "context_length": len(context),
                    "backend": self.llm_config.backend,
                    "json_parsed": parsed_data is not None
                }
            )
    ...
    def _parse_optimizer_response(self, response: str) -> Dict[str, Any]:
        ...
        json_pattern = r'\{[^{}]*"optimized_solution"[^{}]*\}'
        match = re.search(json_pattern, response, re.DOTALL)
        if match:
            try:
        ...
        try:
            solution_match = re.search(
                r'"optimized_solution":\s*"([^"]*(?:\\.[^"]*)*)"',
                response,
                re.DOTALL
            )
            if solution_match:
                # Unescape JSON strings
                solution = solution_match.group(1).replace('\\"', '"').replace('\\n', '\n')
                return {"optimized_solution": solution}
        except Exception:
            pass

        logger.warning("Failed to parse optimizer JSON response, using raw text")
        return None
    ...
    async def _validate_solution(self, user_query: str, context: str, previous_iteration: Dict) -> AgentResponse:
        ...
        try:
            ...
            return AgentResponse(
                ...
                errors_found=[],
                improvements=[],
                metadata={"context_length": len(context), "backend": self.llm_config.backend}
            )
        except Exception as e:
...
class QualityMetrics:
    ...
    def calculate_objective_quality(
        ...
        try:
            ...
            weights = {
                'completeness': 0.40,  # increased from 0.35
                'accuracy': 0.40,
                'length': 0.20,  # increased from 0.15
                'structure': 0.0,
                'polish': 0.0  # removed weight since structure is 0
            }
            if query_type == 'factual':
                pass
            elif query_type == 'definition':
                ...
                weights = {
                    'completeness': 0.25,  # increased from 0.20
                    'accuracy': 0.35,  # increased from 0.30
                    'length': 0.10,
                    'structure': 0.15,
                    'polish': 0.15
                }
            ...
            return {
                ...
                'breakdown': {
                    ...
                    'length': 0.85,
                    'structure': 0.85,
                    # REMOVE THIS LINE: 'context_usage': 0.85,
                    'polish': 0.85
                },
                'weights': {
                    'completeness': 0.20,
                    ...
                    'length': 0.10,
                    'structure': 0.15,
                    # REMOVE THIS LINE: 'context_usage': 0.10,
                    'polish': 0.15
                }
            }
...
class MultiAgentSystem:
    def __init__(self, api_key: str, llm_config: LLMConfig = None):
        self.llm_config = llm_config or LLMConfig()

        # Create agents with appropriate temperatures and backend
        self.generator = AIAgent(api_key, AgentRole.GENERATOR, temperature=0.7, llm_config=self.llm_config)
        self.analyzer = AIAgent(api_key, AgentRole.ANALYZER, temperature=0.2, llm_config=self.llm_config)
        self.optimizer = AIAgent(api_key, AgentRole.OPTIMIZER, temperature=0.4, llm_config=self.llm_config)
        self.validator = AIAgent(api_key, AgentRole.VALIDATOR, temperature=0.1, llm_config=self.llm_config)
    ...
    async def process_query(self, user_query: str, context: str = "",
                            ...
                            file_manager=None) -> MultiAgentSession:  # ADD file_manager parameter
        ...
        if progress_callback:
            progress_callback("context", 0, f"📊 Routing: {complexity.upper()} - {pipeline['description']}")
        ...
        iteration_thresholds = {0: 0.95, 1: 0.92, 2: 0.88, 3: 0.85}

        # Show RAG status
        if progress_callback and file_manager and hasattr(file_manager, 'vector_store'):
            if file_manager.vector_store and file_manager.vector_store.index.ntotal > 0:
                progress_callback("context", 5, f"🔍 RAG: {file_manager.vector_store.index.ntotal} vectors available")
        ...
        while current_iteration < pipeline['max_iterations']:
            ...
            # Generator Agent (ALWAYS RUN) - NOW WITH file_manager
            if progress_callback:
                progress_callback("generator", 20 + (current_iteration * 25), "🔧 Generator Agent (with RAG)...")

            # PASS file_manager to generator
            generator_response = await self.generator.process(
                user_query,
                context,
                iterations[-1] if iterations else None,
                file_manager=file_manager  # ADD THIS
            )
...
class AutonomousMultiAgentAssistant:
    def __init__(self, gemini_api_key: str, llm_config: LLMConfig = None):
        self.console = Console()
        self.file_manager = TextFileManager()
        self.multi_agent_system = MultiAgentSystem(gemini_api_key, llm_config)
        self.running = False
    ...
    async def process_user_message(self, user_id: str, message: str,
        ...
        # ✅ NEW: Pass the query to get_conversation_context for RAG filtering
        context = self.file_manager.get_conversation_context(
            user_id,
            limit=70,  # Ignored when RAG is used
            query=message  # 🔥 THIS IS THE KEY - enables RAG filtering!
        )

        logger.info(f"📊 Context size: {len(context)} chars (~{len(context) // 4} tokens)")

        # Process with multi-agent system
        multi_agent_session = await self.multi_agent_system.process_query(
            message,
            context,  # Now contains ONLY relevant conversations!
            progress_callback=progress_callback,
            file_manager=self.file_manager
        )

        # Save conversation (rest stays same)
...
class RichMultiAgentCLI:
    def __init__(self, api_key: str, llm_config: LLMConfig = None):
        self.console = Console()
        self.assistant = AutonomousMultiAgentAssistant(api_key, llm_config)
    ...
    def display_rag_stats(self):
        """Display RAG/Vector Store statistics"""
        if not self.assistant.file_manager.vector_store:
            self.console.print(Panel(
                "[yellow]⚠️ Vector store not initialized[/yellow]",
                title="🔍 RAG Status",
                border_style="yellow"
            ))
            return
        ...
        progress = self.create_progress()
        task = progress.add_task("[bold cyan]🔍 Loading RAG statistics...[/bold cyan]", total=100)
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            stats = self.assistant.file_manager.vector_store.get_stats()

            for i in range(60, 100, 5):
                progress.update(task, completed=i)
                time.sleep(0.02)

            progress.update(task, completed=100, description="[bold green]✅ RAG stats loaded![/bold green]")
        ...
        stats_table = Table(
            ...
            header_style="bold cyan",
            box=ROUNDED,
            title="RAG Vector Store Statistics"
        )
        ...
        stats_table.add_row("Conversations Indexed", str(stats['conversations']))
        stats_table.add_row("Document Chunks", str(stats['document_chunks']))
        stats_table.add_row("Embedding Dimension", str(stats['dimension']))
        stats_table.add_row("Embedding Model", stats['model'])

        self.console.print(Panel(
            stats_table,
            title="[bold cyan]🔍 RAG Vector Store[/bold cyan]",
            border_style="cyan",
            padding=(1, 1)
        ))
        ...
        • Each conversation & document is converted to vector embeddings
        • When you ask a question, similar past content is retrieved
        • Agents use this retrieved context for better answers
        • Model: {stats['model']} ({stats['dimension']}D embeddings)

        [bold green]✅ Status:[/bold green] RAG is {"ACTIVE" if stats['total_vectors'] > 0 else "EMPTY"}
        """

        self.console.print(Panel(info_text, border_style="green", padding=(1, 2)))

    def search_rag(self):
        """Interactive RAG search"""
        if not self.assistant.file_manager.vector_store:
            self.console.print("[yellow]Vector store not available[/yellow]")
            return

        query = Prompt.ask("[bold cyan]🔍 Enter search query[/bold cyan] (or 'back')").strip()

        if query.lower() in ['back', 'exit', 'cancel']:
            return

        progress = self.create_progress()
        task = progress.add_task("[bold cyan]🔍 Searching vector store...[/bold cyan]", total=100)
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            results = self.assistant.file_manager.vector_store.search(query, top_k=5)

            for i in range(70, 100, 5):
                progress.update(task, completed=i)
                time.sleep(0.02)

            progress.update(task, completed=100, description="[bold green]✅ Search complete![/bold green]")

        if not results:
            self.console.print("[yellow]No similar content found[/yellow]")
            return
        ...
        for i, (metadata, distance) in enumerate(results, 1):
            ...
            if metadata.get('type') == 'document':
                result_text = f"""
    [bold cyan]📄 Document Match {i}[/bold cyan]
    Similarity: [bold green]{similarity:.2%}[/bold green]
    File: {metadata.get('filename')}
    Content: {metadata.get('text', '')[:200]}...
                """
            else:
                result_text = f"""
    [bold green]💬 Conversation Match {i}[/bold green]
    Similarity: [bold green]{similarity:.2%}[/bold green]
    User: {metadata.get('user_message', '')[:100]}...
    Assistant: {metadata.get('ai_response', '')[:100]}...
                """

            self.console.print(Panel(result_text, border_style="cyan", padding=(1, 1)))
    ...
    def display_help(self):
        ...
        • [bold green]download[/bold green]     💾 Download chat history
        ...
        • [bold green]routing[/bold green]      📊 View intelligent routing statistics
        • [bold green]rag[/bold green]          🔍 View RAG/Vector store statistics (NEW!)
        • [bold green]search[/bold green]       🔍 Search vector store (RAG) (NEW!)
        ...
        • Automatic vector indexing of conversations & documents
        • Semantic search for relevant past context
        • Enhanced AI responses using retrieved memories

        [bold cyan]⚡ Shortcuts:[/bold cyan]
        • Type 'r' for routing stats
        • Type 'c' for chat mode
        • Type 'v' for RAG stats
        • Type 's' for RAG search
        • Type 'q' to quit
    ...
    async def process_message_with_progress(self, message: str) -> str:
        ...
        # ✅ FIX: Use try-finally to ensure Live context exits
        try:
            live = Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20)
            live.start()
            ...
            def progress_callback(stage: str, percent: int, description: str = ""):
                if stage in agent_stages:
                    ...
                    current_percent = int(current_task.completed) if current_task.completed else 0

                    # ✅ FIX: Add timeout to prevent infinite loops
                    max_iterations = 100
                    iterations = 0
                    while current_percent < stage_percent and iterations < max_iterations:
                        current_percent += 1
                        progress.update(main_task, completed=current_percent)
            ...
            current_percent = int(progress.tasks[main_task].completed)
            max_iterations = 100
            iterations = 0
            while current_percent < 100 and iterations < max_iterations:
                current_percent += 1
                progress.update(main_task, completed=current_percent)
                time.sleep(0.01)
                iterations += 1
            ...
            # ✅ FIX: Always stop Live context
            live.stop()

        # ✅ FIX: Move response display outside Live context
        self.console.print()  # Separation
